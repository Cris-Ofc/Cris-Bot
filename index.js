...
console.log(color(figlet.textSync('Cris Bot', {
		font: 'Pagga',
...
console.log(color('\nHello, I am Asif, the main Developer of this bot.\n\nThanks for using: Cris Bot','aqua'))
console.log(color('\nYou can follow me on GitHub: Cris-Ofc','aqua'))

    let { version, isLatest } = await fetchLatestBaileysVersion()
    const A17 = A17Connect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['𝗖𝗥𝗜𝗦-𝗕𝗢𝗧:Ⲏꫀɑⲅⲧ Ⲏɑᥴᴋꫀ𝗿 🍁','Safari','1.0.0'],
        auth: state,
        version
    })
...
store.bind(A17.ev)
    ...
    let pa7rick = await A17.sendContact(callerId, global.owner)
    A17.sendMessage(callerId, { text: `Baka! You will be blocked automatically for calling me!`}, { quoted : pa7rick })
    await sleep(8000)
...
I am 𝗖𝗥𝗜𝗦-𝗕𝗢𝗧, Welcome to ${metadata.subject}.
...
`
	...
	for (let i of kon) {
		...
		vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await A17.getName(i + '@s.whatsapp.net')}\nFN:${global.OwnerName}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Click here to chat\nitem2.EMAIL;type=INTERNET:${global.websitex}\nitem2.X-ABLabel:GitHub\nitem3.URL:${global.websitex}\nitem3.X-ABLabel:GitHub\nitem4.ADR:;;${global.location};;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
	A17.sendMessage(jid, { contacts: { displayName: `${list.length} Contact`, contacts: list }, ...opts }, { quoted })
    }
     ...
     * @returns
     */
    A17.send5ButImg = async (jid , text = '' , footer = '', img, but = [], thumb, options = {}) =>{
        let message = await prepareWAMessageMedia({ image: img, jpegThumbnail:thumb }, { upload: A17.waUploadToServer })
     ...
     * @param {*} options 
     */
    A17.sendButtonText = (jid, buttons = [], text, footer, quoted = '', options = {}) => {
        let buttonMessage = {
            text,
            footer,
            buttons,
     ...
     * @returns 
     */
    A17.sendText = (jid, text, quoted = '', options) => A17.sendMessage(jid, { text: text, ...options }, { quoted })
	...
	let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await A17.sendMessage(jid, { image: buffer, caption: caption, ...options }, { quoted })
    }
     ...
     * @returns 
     */
    A17.sendVideo = async (jid, path, caption = '', quoted = '', gif = false, options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await A17.sendMessage(jid, { video: buffer, caption: caption, gifPlayback: gif, ...options }, { quoted })
    }
    ...
    A17.sendAudio = async (jid, path, quoted = '', ptt = false, options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await A17.sendMessage(jid, { audio: buffer, ptt: ptt, ...options }, { quoted })
    }
     ...
     * @returns 
     */
    A17.sendTextWithMentions = async (jid, text, quoted, options = {}) => A17.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
    ...
    A17.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
        ...
        await A17.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted })
        return buffer
    }
	...
	A17.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
       ...
       await A17.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options })
     ...
     * @returns 
     */
    A17.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
     ...
     * @returns 
     */
    A17.copyNForward = async (jid, message, forceForward = false, options = {}) => {
        let vtype
        ...
        await A17.relayMessage(jid, waMessage.message, { messageId:  waMessage.key.id })
        return waMessage
    }


        A17.sendListMsg = (jid, text = '', footer = '', title = '' , butText = '', sects = [], quoted) => {
        let sections = sects
        var listMes = {
        text: text,
        footer: footer,
    ...
    A17.cMod = (jid, copy, text = '', sender = A17.user.id, options = {}) => {
    ...
    A17.getFile = async (PATH, save) => {
        ...
        A17.send5ButGif = async (jid , text = '' , footer = '', gif, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ video: gif, gifPlayback: true }, { upload: A17.waUploadToServer })
        ...
        A17.send5ButVid = async (jid , text = '' , footer = '', vid, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ video: vid }, { upload: A17.waUploadToServer })
        var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
        templateMessage: {
        hydratedTemplate: {
        videoMessage: message.videoMessage,
    ...
    A17.sendFile = async(jid, PATH, fileName, quoted = {}, options = {}) => {
        ...
        await A17.sendMessage(jid, { [type]: { url: pathFile }, mimetype, fileName, ...options }, { quoted, ...options })
        return fs.promises.unlink(pathFile)
    }
